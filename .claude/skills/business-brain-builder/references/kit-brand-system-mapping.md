...
| 02-brand-voice | `07-Voice-Rules.md`. **The five descriptors under "How you sound (5 descriptors)" are Krista's, they ship already filled in, and they are inherited. Do not replace, reorder, or reword them.** The only place their own descriptors may go is the `[LOCAL FILL ...]` line directly beneath those five, and only if their voice genuinely differs. Most people keep the inherited five as they are. Their example phrases have their own destination: the `[LOCAL ADD - phrases your clients hear from you that aren't in the inherited list:]` block under "Words you use". It ships with two numbered blanks. Fill those first, then add further numbered lines beneath rather than dropping anything they gave you | same. Do NOT install the human-writing-enforcement banned-word block from `references/human-writing-enforcement.md` in kit mode - `07-Voice-Rules.md`'s "Words you NEVER use (the universal bans)" section already IS that block, inherited, shared, and already correct. Installing a second copy would duplicate or conflict with it. Skip that install step entirely in kit mode; ask only the personal descriptor and example-phrase questions. |
